...
export default {
  ...
  async pay({ commit }, payload) {
    const response = await api.payments.add(payload);
    commit("pay", response.data);
  },
  async fetchPayments({ commit }, orderId) {
    const response = await api.payments.fetchAll({ orderId });
